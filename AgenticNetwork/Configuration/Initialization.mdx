...
To start building your **Agentic Network** using the Svahnar Agentic AI Framework, begin by defining the top-level key `create_svahnar_agentic_network` in your YAML configuration file. Under this key, define each agent with a unique identifier and specify their properties, including roles.
...
- **`create_svahnar_agentic_network`**: The top-level key that initializes the Agentic Network. This acts as a strict container for all agents in the network to function cohesively. Ensure this key is always present in your YAML configuration.
...
<Tabs>
  <Tab title="Hierarchical Network Flow">
  ...
  <Tab title="Sequential Network Flow">
