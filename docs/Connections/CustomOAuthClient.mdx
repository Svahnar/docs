...
* **Example:** If the provider documentation lists:
  `https://api.notion.com/v1/oauth/token?grant_type=authorization_code`
...
* **Example:** If the provider documentation lists:
  `https://api.notion.com/v1/oauth/authorize?client_id=YOUR_CLIENT_ID&response_type=code&owner=user`
...
The endpoint SVAHNAR calls to **introspect/inspect the status of an access token** directly on the Authorization Server. This is defined in [RFC 7662](https://datatracker.ietf.org/doc/html/rfc7662).
...
:::info Dynamic Registration Prerequisite
