...
→ **[Creating a Custom OAuth Client](./CustomOAuthClient)** — full guide with field-by-field explanations and provider-specific examples.
...
See the [Custom OAuth Clients](./CustomOAuthClient#-using-custom-providers-with-mcp-servers) guide for full details and examples.
...
For **MCP servers** that require authentication, use `oauth_provider_id` with a [custom OAuth client](./CustomOAuthClient):
