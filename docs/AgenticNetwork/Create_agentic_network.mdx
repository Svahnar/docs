...
Begin by defining the top-level key `create_svahnar_agentic_network` to initialize the Agentic Network. Under this key, you'll define each agent with a unique identifier.
...
- **provider**: Specifies the LLM provider (e.g., ChatOpenAI).
