...
To begin, initialize the **Agentic Network** by defining the top-level key `create_vertical_agent_network`. This key acts as a container for all agents in your network.
...
For detailed information, refer to [Initialization Documentation](/docs/AgenticNetwork/Configuration/Initialization).


</Step>
<Step title="Configure the LLM for Reasoning">
The next step is to configure the **Large Language Model (LLM)** that powers your agent. Configure the model, specific variant, and other hyperparameters. For example:
...
- **model**: Specifies the model variant (`gpt-5` in this case).
...
For more details, visit [LLM Configuration Documentation](/docs/AgenticNetwork/Configuration/LLM).
...
For additional information, check out [Tools Documentation](/docs/AgenticNetwork/Configuration/Tools).
...
Learn more about edges at [Incoming and Outgoing Edges Documentation](/docs/AgenticNetwork/Configuration/Edges).
