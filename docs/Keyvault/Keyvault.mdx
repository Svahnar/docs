...
*   **Agent Console**: In Agent Node fields or Tool Config fields.
    ```yaml
    api_key: ${tavily_api_key}
    ```

*   **YAML Configurations**: When adding keys in YAML, enclose the placeholder in **double quotes** to ensure proper parsing.
    ```yaml
    api_key: "${tavily_api_key}"
    ```

### Step 3: Revoke a Secret
...
> **Use Case:** Authenticating a tool.
