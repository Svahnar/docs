...
This tool uses **OAuth 2.0** with **Refresh Tokens**.

* **First Run:** You must explicitly click **Authenticate** in the tool configuration UI to generate the initial tokens.
* **Maintenance:** Access tokens are refreshed automatically, but the long-lived refresh token may expire approximately every **90 days**. If the tool stops working, simply re-run the Authenticate flow.
...
For security, never hardcode your Client Secret in plain text files. Use environment variables (like `${client_secret}`) or the Svahnar Secrets Manager.
...
* If the `offline_access` permission is missing, Microsoft will not issue a refresh token, and the connection will fail after 1 hour.