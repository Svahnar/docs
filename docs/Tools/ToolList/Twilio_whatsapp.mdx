---
title: "Twilio WhatsApp Tool Integration Guide | SVAHNAR Docs"
description: "Send automated WhatsApp messages from your SVAHNAR AI agents using Twilio's WhatsApp API."
keywords: ['twilio whatsapp', 'whatsapp messaging', 'svahnar', 'messaging automation', 'ai agent', 'developer documentation', 'agent tools', 'tool integration', 'api integration']
...
:::tip
If you are using the template message, you need to provide `CONTENT_SID` and `CONTENT_VARIABLES`. and require to use JSON format for the `CONTENT_VARIABLES` check out our [JSON to YAML conversion guide](/docs/additional/YAML_Conversion_Guide).
:::
