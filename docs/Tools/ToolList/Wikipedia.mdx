...
Your agent needs to send a specific JSON payload to trigger the search. Ensure your agent's system prompt encourages it to generate a `query`.
...
* **Success:** A string containing the summary of the requested topic.
* **Usage:** The agent uses this text to answer user questions or perform further reasoning.
</Step>
...
The tool automatically merges the runtime payload (the `query` from the agent) with your static YAML configuration. This means you can hardcode the `lang` in your YAML while letting the agent dynamically decide the `query`.
...
* The agent attempted to call the tool without specifying what to search for. Update your agent's instructions to explicitly mention providing a `query` string.


* **RuntimeError / Connection Issues**
* This usually indicates a failure to reach the Wikipedia API. Check your internet connection or verify if the Wikipedia API is currently down.


* **Irrelevant Results**
* If the summary returned is not helpful, consider increasing `top_k_results` in the config to give the agent a broader set of snippets to read.